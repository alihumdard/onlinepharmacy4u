...
@section('title', 'Prescription Med General Questions')
@section('content')
    <!-- main stated -->
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            <h1><a href="javascript:void(0);" onclick="window.history.back();" class="btn btn-primary-outline fw-bold "><i
                        class="bi bi-arrow-left"></i> Back</a> | Prescription Med General Questions</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="/">Home</a></li>
                    <li class="breadcrumb-item">Pages</li>
                    <li class="breadcrumb-item active">Prescription Med General Questions</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
                    ...
                    <div class="card">
                        <div class="card-header mt-3" id="tbl_buttons"
                            ...
                            <button type="button" class="btn btn-primary" id="createQuestionButton" style="color: black;">
                                Create Prescription Med General Questions
                            </button>
                        </div>
                        <div class="card-body">
    ...
    <!-- End #main -->
    <form id="edit_form" action="{{ route('Add.Prescription.Med.Questions') }}" method="post">
        @csrf
        <input id="edit_form_id_input" type="hidden" value="" name="id">
    </form>
    <!-- End #main -->
    <form id="delete_form" action="{{ route('Delete.Prescription.Med.Question') }}" method="post">
        @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('createQuestionButton').addEventListener('click', function() {
                window.location.href = '{{ route('Add.Prescription.Med.Questions') }}';
            });
        });
    </script>
    <script>
        ...
        $("#sortable").sortable({
            update: function(event, ui) {
                ...
                $.ajax({
                    url: '{{ route('Update.PrescriptionQuestion.Order') }}',
                    method: 'POST',
                    data: {
                        order: order,
                        _token: '{{ csrf_token() }}'
